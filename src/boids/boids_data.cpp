...
BoidsData::BoidsData(const std::shared_ptr<Render::Mesh::Mesh>& mesh, const std::shared_ptr<BoidsParams>& params)
    ...
    data.resize(params->boids * instance_float_size);
    for (int i = 0; i < params->boids; i++) {
        ...
        data[i * instance_float_size]     = pos.x;
        data[i * instance_float_size + 1] = pos.y;
        data[i * instance_float_size + 2] = pos.z;
        data[i * instance_float_size + 3] = 0.0f; // Padding
        // Velocity data
        ...
        data[i * instance_float_size + 4] = vel.x;
        data[i * instance_float_size + 5] = vel.y;
        data[i * instance_float_size + 6] = vel.z;
        data[i * instance_float_size + 7] = 0.0f; // Padding
        // grid cell
        glm::uvec3 grid_cell = glm::ivec3(pos / params->view_range);
        data[i * instance_float_size + 8] = grid_cell.x;
        data[i * instance_float_size + 9] = grid_cell.y;
        data[i * instance_float_size + 10] = grid_cell.z;
        data[i * instance_float_size + 11] = 0.0f;
    }
